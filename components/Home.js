...
class MarketStock extends React.Component{
	render(){
		...
		const changeAmount= student.stockValueGraphPoints[student.stockValueGraphPoints.length-1].y-student.stockValueGraphPoints[student.stockValueGraphPoints.length-Math.min(7,student.stockValueGraphPoints.length)].y;
		const changePercent= changeAmount/student.stockValueGraphPoints[student.stockValueGraphPoints.length-Math.min(7,student.stockValueGraphPoints.length)].y;
			...
			<div className='market-stock' style={{width:String(width)+"px"}} onClick={this.props.onClick}>
				...
				<div className="stock-info" style={{width:String(width-150)+"px"}}>
					...
					<div className="stock-info-stats">
						<div className="stock-info-stats-value">{"$"+numberWithCommas(student.stockValueGraphPoints[student.stockValueGraphPoints.length-1].y.toFixed())}</div>
...
class Dashboard extends React.Component{
	constructor(props){
		...
		users.sort((a,b)=> (b.changePercent - a.changePercent));
		...
		this.state={
			...
			stockExchangeBuyAmount: "",
			stockExchangeSellAmount: "",
		};
	}
	...
	createMarket(stockWidth){ //*****add key's to market items*****
		...
		if(this.state.nameFilter===""){
			for(var i=0; i<users.length; i++)
				if(this.state.user.UID !== users[i].UID)
			...
			for(var j=0; j<users.length; j++){
				...
				if((name[0].toUpperCase().startsWith(filter.toUpperCase()) || name[1].toUpperCase().startsWith(filter.toUpperCase())) && this.state.user.UID !== users[j].UID)
	...
	getChange(user){
		return user.stockValueGraphPoints[user.stockValueGraphPoints.length-1].y - user.stockValueGraphPoints[user.stockValueGraphPoints.length-Math.min(7,user.stockValueGraphPoints.length)].y;
	}

	handleStockExchangeBuyInput(evt){
		const amount = (evt.target.validity.valid) ? evt.target.value : Math.floor(this.state.user.availableFunds/this.state.stockProfileStudent.stockValueGraphPoints[this.state.stockProfileStudent.stockValueGraphPoints.length-1].y);
		this.setState({stockExchangeBuyAmount:amount});
	}
	handleStockExchangeSellInput(evt){
		const amount = (evt.target.validity.valid) ? evt.target.value : this.portfolioStockQuantity;
		this.setState({stockExchangeSellAmount:amount});
	}
		...
		if(active === "change"){ //currently sorting by change
			if(buttonName === "change"){ //user clicked change again
				if(direction === "down"){//switch to ascending change sort
					this.state.users.sort((a,b)=> (this.getChange(a) - this.getChange(b)));
					//(a.)
					this.setState({sortDirection:"up"});
				}
				else {//direction is up so switch to descending change sort
					this.state.users.sort((a,b)=> (this.getChange(b) - this.getChange(a)));
					this.setState({sortDirection:"down"});
				}
			}
			else if(buttonName === "price"){ //switch to descending price sort
				this.state.users.sort((a,b)=> (b.stockValueGraphPoints[b.stockValueGraphPoints.length-1].y - a.stockValueGraphPoints[a.stockValueGraphPoints.length-1].y));
				this.setState({sortDirection:"down"});
			}
			...
			if(buttonName === "change"){
				...
				if(direction === "down"){//switch to ascending price sort
					this.state.users.sort((a,b)=> (a.stockValueGraphPoints[a.stockValueGraphPoints.length-1].y - b.stockValueGraphPoints[b.stockValueGraphPoints.length-1].y));
					this.setState({sortDirection:"up"});
				}
				else {
					this.state.users.sort((a,b)=> (b.stockValueGraphPoints[b.stockValueGraphPoints.length-1].y - a.stockValueGraphPoints[a.stockValueGraphPoints.length-1].y));
			...
			if(buttonName === "change"){
				...
				if(direction === "down"){//switch to ascending price sort
					this.state.users.sort((a,b)=> b.name.localeCompare(a.name));
					this.setState({sortDirection:"up"});
				}
				else {
					this.state.users.sort((a,b)=> a.name.localeCompare(b.name));
	...
	render(){
		...
		const marketGridStyle = {
			height:Math.ceil(this.state.users.length/(stocksPerRow)) * 205,
		...
		if(this.state.stockProfileStudent!= null)
			for(var k=0; k<this.state.user.portfolioStocks.length; k++)
				if(this.state.user.portfolioStocks[k].UID === this.state.stockProfileStudent.UID){
					this.portfolioStockQuantity = this.state.user.portfolioStocks[k].quantity;
					break;
				}
						...
						{this.state.stockExchangeMode==="buy"?
							<div style={{width: "100%"}}>
								<div className="stock-exchange-panel-info">
									...
									<div style={{fontSize:"18px"}}>{"$"+numberWithCommas(this.state.user.availableFunds.toFixed(2))}</div>
								</div>
								<input 	id="stock-exchange-stocks-to-buy" type="number" 
										min="0" max={String(Math.floor(this.state.user.availableFunds/this.state.stockProfileStudent.stockValueGraphPoints[this.state.stockProfileStudent.stockValueGraphPoints.length-1].y))} 
										placeholder="Shares to Buy" onInput={this.handleStockExchangeBuyInput.bind(this)} value={this.state.stockExchangeBuyAmount}
								/>
								<div className="stock-exchange-panel-info">
									<div style={{marginLeft: "10px"}}>Market Price</div>
									<div style={{fontSize:"18px"}}>{"$"+numberWithCommas(this.state.stockProfileStudent.stockValueGraphPoints[this.state.stockProfileStudent.stockValueGraphPoints.length-1].y.toFixed(2))}</div>
								</div>
								<div className="stock-exchange-panel-info">
									<div style={{marginLeft: "10px"}}>Total Cost</div>
									<div style={{fontSize:"18px"}}>{
										this.state.stockExchangeBuyAmount=== "" ?
										"$0.00"
										:
										<div>{"$"+numberWithCommas((this.state.stockProfileStudent.stockValueGraphPoints[this.state.stockProfileStudent.stockValueGraphPoints.length-1].y*this.state.stockExchangeBuyAmount).toFixed(2))}</div>	
									}</div>
								</div>
						...
						:
							<div style={{width: "100%"}}>
								...
								<input id="stock-exchange-stocks-to-buy" type="number"
										...
										placeholder="Shares to Sell" onInput={this.handleStockExchangeSellInput.bind(this)} value={this.state.stockExchangeSellAmount}
								/>
								<div className="stock-exchange-panel-info">
									<div style={{marginLeft: "10px"}}>Market Price</div>
									<div style={{fontSize:"18px"}}>{"$"+numberWithCommas(this.state.stockProfileStudent.stockValueGraphPoints[this.state.stockProfileStudent.stockValueGraphPoints.length-1].y.toFixed(2))}</div>
								...
								<div className="stock-exchange-panel-info">
									...
									<div style={{fontSize:"18px"}}>{
										this.state.stockExchangeSellAmount==="" ?
										"$0.00"
										:
										<div>{"$"+numberWithCommas((this.state.stockProfileStudent.stockValueGraphPoints[this.state.stockProfileStudent.stockValueGraphPoints.length-1].y*this.state.stockExchangeSellAmount).toFixed(2))}</div>
									}</div>
								</div>
...
class SideBar extends React.Component{
	constructor(props){
		...
		const stockChangeAmount = user.stockValueGraphPoints[user.stockValueGraphPoints.length-1].y-user.stockValueGraphPoints[user.stockValueGraphPoints.length-Math.min(7,user.stockValueGraphPoints.length)].y;

		this.state={
			studentName: user.name,
			netWorth: user.netWorthGraphPoints[user.netWorthGraphPoints.length-1].y,
			portfolioValue: user.portfolioValue,
			availableFunds: user.availableFunds,
			profitLoss: (user.netWorthGraphPoints[user.netWorthGraphPoints.length-1].y-user.netWorthGraphPoints[user.netWorthGraphPoints.length-Math.min(7,user.netWorthGraphPoints.length)].y),

			stockValue: user.stockValueGraphPoints[user.stockValueGraphPoints.length-1].y,
			stockChangeAmount: stockChangeAmount,
			stockChangePercent: stockChangeAmount/user.stockValueGraphPoints[user.stockValueGraphPoints.length-Math.min(7,user.stockValueGraphPoints.length)].y,
		};
	}
	...
	render(){
		...
		return (
			<div className = "side-bar" style={{height: sideBarHeight}}>
				...
				<SideBarInfo value={this.state.availableFunds} title="Available Funds" icon="fa fa-credit-card"/>
	...
	render(){
		return (
			<div className = "home">
				...
				<Dashboard selfUser={this.props.selfUser} users={this.props.users}/>
			</div>
		);
	}
}
